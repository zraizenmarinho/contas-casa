...
function ListaView({ contas, mesRef, onAdd, onEdit, onDelete, onStatusChange, onDuplicarMes }) {
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          {pill('Todos',    filtroStatus === 'todos',    () => setFiltroStatus('todos'),    '#18181A')}
          {pill('Pendente', filtroStatus === 'pendente', () => setFiltroStatus('pendente'), '#D4900A')}
          {pill('Pago',     filtroStatus === 'pago',     () => setFiltroStatus('pago'),     '#2D9D6B')}
      ...
      {filtered.length > 0 && (
        <div style={{
          ...
          <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>{filtered.length} conta{filtered.length > 1 ? 's' : ''}</span>
